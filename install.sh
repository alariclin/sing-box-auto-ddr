...
checkSystem() {
    ...
    if [[ -z ${release} ]]; then
        echo -e "${RED}\n[!] 本脚本不支持此系统，请更换 Ubuntu/Debian/CentOS 后重试。\n${NC}"
        exit 1
    fi
}
...
release_ports() {
    echo -e "${YELLOW}[*] 正在执行内核级端口死锁清理...${NC}"
...
deploy_xray() {
    ...
    checkSystem; check_env; pre_install_setup "$MODE"; release_ports; get_architecture
...
deploy_singbox() {
    local MODE=$1; clear; echo -e "${BOLD}${GREEN} 部署 Sing-box [$MODE] ${NC}"
    checkSystem; check_env; pre_install_setup "$MODE"; release_ports; get_architecture
...
clean_uninstall() {
    ...
    echo -e "${GREEN}✔ 物理清场完成！VPS 已恢复纯净。${NC}"; exit 0
}
...
tune_vps() {
    clear; echo -e "${CYAN}正在执行 VPS 内核级网络优化 (BBR + 系统限制提升)...${NC}"
